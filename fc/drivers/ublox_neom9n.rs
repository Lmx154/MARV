...
impl GpsModule {
    ...
    pub fn init(
        ...
        let mut uart = UartPeripheral::new(uart0, (tx_pin, rx_pin), resets)
            .enable(uart_config, clocks.peripheral_clock.freq())?;
        
        // Wake the module
        uart.write_full_blocking(&[0xFF]);

        // CFG-PRT: Set UART to UBX-only output at 38400 baud
        let cfg_prt: [u8; 28] = [0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x91, 0x84];
        uart.write_full_blocking(&cfg_prt);

        // CFG-MSG: Enable NAV-PVT on UART
        let cfg_msg: [u8; 16] = [0xB5, 0x62, 0x06, 0x01, 0x08, 0x00, 0x01, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x18, 0xE1];
        uart.write_full_blocking(&cfg_msg);

        // CFG-CFG: Save configuration to flash
        let cfg_cfg: [u8; 21] = [0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1F, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x5E, 0xCC];
        uart.write_full_blocking(&cfg_cfg);
