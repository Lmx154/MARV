//! Simple GPS Module
//! Parses UBX NAV-PVT messages to extract GPS data
...
impl GpsModule {
    ...
    pub fn init(
        ...
        // Send dummy byte to wake the module if in power save mode
        uart.write_full_blocking(&[0xFF]);

        // Configure UART1 to enable UBX output (inProto: UBX+NMEA, outProto: UBX)
        let cfg_prt: [u8; 28] = [0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8D, 0xA6];
        uart.write_full_blocking(&cfg_prt);

        // Enable UBX-NAV-PVT message on UART1 at rate 1
        let cfg_msg: [u8; 16] = [0xB5, 0x62, 0x06, 0x01, 0x08, 0x00, 0x01, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x18, 0x5B];
